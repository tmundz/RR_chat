...
async fn main() -> std::io::Result<()> {
    ...
    let port = 8080;
    let chatapp = HttpServer::new(move || {
        ...
        App::new()
            ...
            .service(routes::get_conservation_by_id)
            .service(routes::get_rooms)
            .service(Files::new("/", "./statix"))
    }).workers(2).bind((address, port))?.run();
    println!("Server running http://{address}:{port}/");
    app.await
}
